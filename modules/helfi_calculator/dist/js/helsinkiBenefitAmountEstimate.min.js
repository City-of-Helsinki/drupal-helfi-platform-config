(() => {
  ...
  // assets/js/helsinkiBenefitAmountEstimate/_form.js
      ...
      items: [
        ...
        },
        {
          input_integer: {
            ...
            min: 0,
            max: 454,
            // Vacation money per month is half monthly_pay divided with 11 months; also adjusted in code when monthly_pay changes
            size: 12,
            required: false,
            strip: "[€eE ]",
  ...
  var form_default = { getFormData };

  // assets/js/helsinkiBenefitAmountEstimate/_translations.js
  var translations = {
    heading_employer: {
  ...
  // assets/js/helsinkiBenefitAmountEstimate/helsinkiBenefitAmountEstimate.js
  var _HelsinkiBenefitAmountEstimate = class _HelsinkiBenefitAmountEstimate {
    constructor(id, settings) {
      this.id = id;
      const config = JSON.parse(settings);
      const checkConfiguration = /* @__PURE__ */ __name(() => {
      ...
      checkConfiguration();
      const getFormData2 = /* @__PURE__ */ __name(() => form_default.getFormData(this.id, this.t), "getFormData");
      const isEmployeeAssociation = /* @__PURE__ */ __name(() => this.calculator.getFieldValue("company_type") === "association", "isEmployeeAssociation");
      const isEmployeeBusiness = /* @__PURE__ */ __name(() => this.calculator.getFieldValue("company_type") === "business", "isEmployeeBusiness");
      const hasBusinessActivities = /* @__PURE__ */ __name(() => isEmployeeAssociation() && this.calculator.getFieldValue("association_has_business_activities"), "hasBusinessActivities");
      const getMonthlyPay = /* @__PURE__ */ __name(() => parseInt(this.calculator.getFieldValue("monthly_pay"), 10), "getMonthlyPay");
      const getVacationMoney = /* @__PURE__ */ __name(() => {
      ...
      const setVacationMoneyMax = /* @__PURE__ */ __name(() => {
        this.calculator.getElement("vacation_money").dataset.max = Math.floor(getMonthlyPay() / 2 / 11);
      }, "setVacationMoneyMax");
      const getOtherExpenses = /* @__PURE__ */ __name(() => config.SALARY_OTHER_EXPENSES_PERCENTAGE * getMonthlyPay(), "getOtherExpenses");
      const getSalaryWithExpenses = /* @__PURE__ */ __name(() => [getMonthlyPay(), getVacationMoney()].reduce(
        (acc, value) => parseInt(acc, 10) + parseInt(value, 10),
        getOtherExpenses()
      ), "getSalaryWithExpenses");
      const getStateAidPercentage = /* @__PURE__ */ __name(() => {
        if (isEmployeeBusiness() || hasBusinessActivities()) {
          return config.STATE_AID_PERCENTAGES[1];
        }
        return config.STATE_AID_PERCENTAGES[0];
      }, "getStateAidPercentage");
      const getStateAidAmount = /* @__PURE__ */ __name(() => getStateAidPercentage() * getSalaryWithExpenses(), "getStateAidAmount");
      const getHelsinkiBenefitAmount = /* @__PURE__ */ __name(() => getStateAidAmount(), "getHelsinkiBenefitAmount");
      const formatCurrency = /* @__PURE__ */ __name((number) => this.calculator.formatFinnishEuroCents(number), "formatCurrency");
      const debugUpdate = /* @__PURE__ */ __name(() => {
        if (true) {
          const data = {
      ...
      this.calculator = window.helfiCalculator({ name: "helsinkiBenefitAmountEstimate", translations: translations_default });
  ...
  __name(_HelsinkiBenefitAmountEstimate, "HelsinkiBenefitAmountEstimate");
  var HelsinkiBenefitAmountEstimate = _HelsinkiBenefitAmountEstimate;
  window.helfiCalculator = window.helfiCalculator || {};
  window.helfiCalculator.helsinki_benefit_amount_estimate = (id, settings) => new HelsinkiBenefitAmountEstimate(id, settings);
})();
//# sourceMappingURL=helsinkiBenefitAmountEstimate.min.js.map
