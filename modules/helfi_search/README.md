...
`deboost_bundles`, `deboost_factor`, and `similarity` are added to `config_ignore` by `ConfigIgnoreHook`, so they can be tuned on a running site.
...
| `similarity`      | float (0.0–1.0) | Cosine-similarity passed to the KNN query. Hits below it are dropped. Elasticsearch calculates the document `_score` as `(similarity + 1) / 2`. |
...
```sh
drush config:set helfi_search.settings similarity 0.4
